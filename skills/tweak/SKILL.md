---
name: tweak
description: Review and fix issues from annotations.txt
---

Review the annotations file at @annotations.txt which contains code review feedback and issues to address.

For each annotation:
1. Read the referenced file and line numbers
2. If the annotation is a question, answer it
3. Understand the requested change from the comment
4. Make the necessary code changes to fix the issue
5. Verify the fix is complete

After all issues have been addressed:
1. Delete the original `annotations.txt` file
2. Stage the changed files with `gt add -A`
3. Use /graphite-modify to amend staged changes to the current commit
4. Use /graphite-submit to push and update the PR

Work through each annotation systematically and confirm when complete.
